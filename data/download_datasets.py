...
Downloads all 3 real datasets used by the system:
  ...
  2. MIT-BIH ECG Arrhythmia (Kaggle: shayanfazeli/heartbeat)
     → mitbih_train.csv (87,554 rows × 188 cols: 187 signal samples + 1 AAMI label)
     → mitbih_test.csv  (21,892 rows × 188 cols)
     NOTE: The dataset sadmansakib7/ecg-arrhythmia-classification-dataset contains
           raw multi-database files (MIT-BIH, INCART, SCD Holter, etc.) in a
           different format and does NOT ship mitbih_train/test.csv.
           The correct pre-processed 5-class CSV source is shayanfazeli/heartbeat.
...
KAGGLE_DATASETS = {
    ...
    # shayanfazeli/heartbeat is the correct pre-processed MIT-BIH source.
    # It ships mitbih_train.csv (87,554 rows) and mitbih_test.csv (21,892 rows)
    # where each row = 187 normalised ECG signal samples + 1 AAMI class label (0-4).
    # sadmansakib7/ecg-arrhythmia-classification-dataset ships raw waveform files
    # from multiple databases (MIT-BIH, INCART, SCD Holter, MIT-BIH Supraventricular)
    # and does NOT contain the mitbih_train/test CSV format required here.
    "mitbih": "shayanfazeli/heartbeat",
}
...
def download_mitbih():
    print_header("Dataset 2: MIT-BIH ECG Arrhythmia (shayanfazeli/heartbeat)")
    print("  Pre-processed 5-class AAMI format: 187 signal samples + 1 label per row")
    print("  Train: 87,554 beats | Test: 21,892 beats | Classes: N/S/V/F/Q")
    ...
    print()
    print("  NOTE: Do NOT use sadmansakib7/ecg-arrhythmia-classification-dataset —")
    print("        that dataset contains raw multi-database waveform files,")
    print("        not the pre-processed mitbih_train/test CSV format.")
    return False
