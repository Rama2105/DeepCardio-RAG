...
# ─────────────────────────────────────────────────────────────────────────────
# ECGArrhythmiaPipeline — unified wrapper used by the Colab notebook / main.py
# Provides: load() → preprocess() → train(epochs) → predict(signal)
# ─────────────────────────────────────────────────────────────────────────────
class ECGArrhythmiaPipeline:
    """
    Thin facade over ECGArrhythmiaDataLoader + ECGArrhythmiaPredictor.
    Exposes the load / preprocess / train interface expected by
    the Colab notebook (Step 7) and the /api/ecg/train endpoint.
    """

    def __init__(self):
        self.loader    = ECGArrhythmiaDataLoader()
        self.predictor = ECGArrhythmiaPredictor()
        self.train_df  = None
        self.test_df   = None
    ...
    def load(self):
        """
        Load mitbih_train.csv + mitbih_test.csv (auto-downloads if missing).
        Returns (train_df, test_df) tuple.
        If test CSV is absent, performs an 80/20 stratified split automatically.
        """
        self.train_df, self.test_df = self.loader.load()
        return self.train_df, self.test_df

    # ------------------------------------------------------------------
    def preprocess(self):
        """
        Signal scaling and tensor conversion are handled inside
        ECGArrhythmiaPredictor.train(), so this is a no-op validation step.
        """
        if self.train_df is None:
    ...
    # ------------------------------------------------------------------
    def train(self, epochs: int = 30) -> dict:
        """
        Train the 1D-CNN on mitbih_train.csv, evaluate on mitbih_test.csv.
        Returns metrics dict with accuracy, f1_macro, auc_roc_ovr, etc.
        """
        if self.train_df is None:
    ...
    # ------------------------------------------------------------------
    def predict(self, signal: np.ndarray) -> dict:
        """Run inference on a single ECG signal array (shape: 187,)."""
